...
def cal_laplacian_loss(foreground_img, foreground_mask, blended_img, background_mask):
    ...
    foreground_valid = foreground_mask.expand(-1, foreground_img.shape[1], -1, -1) > 0.5
    background_valid = background_mask.expand(-1, blended_img.shape[1], -1, -1) > 0.5

    foreground_values = foreground_laplacian[foreground_valid]
    blended_values = blended_laplacian[background_valid]
    if foreground_values.numel() == 0 or blended_values.numel() == 0:
        return loss

    pixel_count = min(foreground_values.numel(), blended_values.numel())
    loss = F.mse_loss(blended_values[:pixel_count], foreground_values[:pixel_count])

...
def mean_knn_distance(points: torch.Tensor, k: int, chunk_size: int = 1024) -> torch.Tensor:
    """Compute mean KNN distances without relying on PyTorch3D."""
    n_points = points.shape[0]
    if n_points <= 1:
        return torch.ones((n_points, 1), dtype=points.dtype, device=points.device)
    ...
    for start in range(0, n_points, chunk_size):
        ...
        dists = torch.cdist(chunk, points, p=2)
        row_idx = torch.arange(end - start, device=points.device)
        col_idx = torch.arange(start, end, device=points.device)
        dists[row_idx, col_idx] = float("inf")
        knn_dists = torch.topk(dists, k=k, dim=1, largest=False).values
        mean_dists.append(knn_dists.mean(dim=1, keepdim=True))
    return torch.cat(mean_dists, dim=0)
...
class GaussianModel(nn.Module):
    ...
    def _init_scales(self, points3D_xyz: torch.Tensor) -> None:
        ...
        mean_dists = mean_knn_distance(points3D_xyz, K) * 2.0
    ...
    def _init_opacities(self) -> None:
        ...
        # Start from a moderate opacity to avoid unstable alpha saturation.
        self.opacities = nn.Parameter(
            -2.0 * torch.ones((self.n_points, 1), dtype=torch.float32)
        )
    ...
    def compute_covariance(self) -> torch.Tensor:
        ...
        scales = torch.exp(self.scales).clamp(min=1e-2, max=2.0)
        S = torch.diag_embed(scales)

        # Compute covariance
        covs3d = torch.bmm(torch.bmm(R, S), torch.bmm(S, R.transpose(1, 2)))

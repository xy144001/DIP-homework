...
class GaussianRenderer(nn.Module):
    ...
    def compute_projection(
        ...
        means2D = torch.stack(
            [
                fx * cam_points[:, 0] / z + cx,
                fy * cam_points[:, 1] / z + cy,
            ],
            dim=-1,
        )
        ...
        J_proj[:, 0, 0] = fx / z
        J_proj[:, 0, 2] = -fx * cam_points[:, 0] / (z * z)
        J_proj[:, 1, 1] = fy / z
        J_proj[:, 1, 2] = -fy * cam_points[:, 1] / (z * z)
        
        # Transform covariance to camera space
        covs_cam = torch.bmm(torch.bmm(R.unsqueeze(0).expand(N, -1, -1), covs3d), R.T.unsqueeze(0).expand(N, -1, -1))
    ...
    def compute_gaussian_values(
        ...
        eigvals, eigvecs = torch.linalg.eigh(covs2D)
        eigvals = eigvals.clamp(min=eps)
        inv_covs = torch.bmm(
            eigvecs,
            torch.bmm(torch.diag_embed(1.0 / eigvals), eigvecs.transpose(1, 2)),
        )
        det_covs = eigvals.prod(dim=-1).clamp(min=eps)
        quad_form = torch.einsum("nhwc,ncd,nhwd->nhw", dx, inv_covs, dx)
        norm = (1.0 / (2.0 * np.pi * torch.sqrt(det_covs))).view(N, 1, 1)
        gaussian = norm * torch.exp(-0.5 * quad_form)
        gaussian = torch.nan_to_num(gaussian, nan=0.0, posinf=0.0, neginf=0.0).clamp_(0.0, 1.0)
    ...
    def forward(
        ...
        transmittance = torch.cumprod(
            torch.cat([torch.ones_like(alphas[:1]), 1.0 - alphas[:-1] + 1e-6], dim=0),
            dim=0,
        )
        weights = alphas * transmittance
        
        # 8. Final rendering
        rendered = (weights.unsqueeze(-1) * colors).sum(dim=0).clamp(0.0, 1.0)  # (H, W, 3)

...
class GaussianTrainer:
    ...
    def sanitize_parameters(self):
        """Clamp learnable parameters back to a numerically safe range."""
        with torch.no_grad():
            self.model.positions.data = torch.nan_to_num(self.model.positions.data)
            self.model.colors.data = torch.nan_to_num(self.model.colors.data).clamp(-8.0, 8.0)
            self.model.opacities.data = torch.nan_to_num(self.model.opacities.data, nan=-2.0).clamp(-6.0, 6.0)
            self.model.scales.data = torch.nan_to_num(self.model.scales.data).clamp(np.log(1e-2), np.log(2.0))
            self.model.rotations.data = torch.nan_to_num(self.model.rotations.data)
            zero_norm = self.model.rotations.data.norm(dim=-1) < 1e-8
            self.model.rotations.data[zero_norm, 0] = 1.0
    ...
    def train_step(self, batch: dict, in_train = True) -> float:
        ...
        if not torch.isfinite(loss):
            ...
            return 0.0, rendered_images.detach()
